...
async function syncWithBackend() {
    try {
        ...
        if (!response.ok) {
            ...
            alert('🚨 ERROR FATAL DEL SERVIDOR:\n\n' + errorText);
...
function updateStudentView() {
    ...
    const powerBadge = document.getElementById('studentSuperPowerBadge');
    if (powerBadge) {
        powerBadge.innerHTML = `<i class="fas fa-bolt"></i> ${currentUser.superPower || 'Poder Oculto'}`;
    }
...
// ==========================================
// EDITOR DE PERFIL DE ESTUDIANTE
// ==========================================

let studentEditCurrentAvatarIndex = 0;

function openStudentEditModal() {
    if (!currentUser) return;
    
    document.getElementById('studentEditSuperPower').value = currentUser.superPower || '';
    
    // Buscar el índice del avatar actual
    studentEditCurrentAvatarIndex = AVAILABLE_AVATARS.indexOf(currentUser.avatar);
    if (studentEditCurrentAvatarIndex === -1) studentEditCurrentAvatarIndex = 0;
    
    document.getElementById('studentEditAvatarDisplay').textContent = AVAILABLE_AVATARS[studentEditCurrentAvatarIndex];
    
    const modal = new bootstrap.Modal(document.getElementById('studentEditModal'));
    modal.show();
}

function prevStudentEmoji() {
    studentEditCurrentAvatarIndex--;
    if (studentEditCurrentAvatarIndex < 0) studentEditCurrentAvatarIndex = AVAILABLE_AVATARS.length - 1;
    document.getElementById('studentEditAvatarDisplay').textContent = AVAILABLE_AVATARS[studentEditCurrentAvatarIndex];
}

function nextStudentEmoji() {
    studentEditCurrentAvatarIndex++;
    if (studentEditCurrentAvatarIndex >= AVAILABLE_AVATARS.length) studentEditCurrentAvatarIndex = 0;
    document.getElementById('studentEditAvatarDisplay').textContent = AVAILABLE_AVATARS[studentEditCurrentAvatarIndex];
}
...
async function saveStudentProfile() {
    ...
    const newPower = document.getElementById('studentEditSuperPower').value;
    const newAvatar = AVAILABLE_AVATARS[studentEditCurrentAvatarIndex];
    
    // Usar la API de héroes para actualizar
    const heroData = {
        ...currentUser,
        superPower: newPower,
        avatar: newAvatar
    ...
    try {
        const result = await heroAPI.update(currentUser.id, heroData);
        if (result) {
            currentUser.superPower = newPower;
            currentUser.avatar = newAvatar;
            
            updateStudentView(); // Refrescar UI visual
            
            const modalEl = document.getElementById('studentEditModal');
            const modal = bootstrap.Modal.getInstance(modalEl);
            if (modal) modal.hide();
            
            showSuccessAnimation('¡Tu perfil ha sido actualizado! 🦸‍♂️✨');
            
            // Sincronizar silenciosamente para actualizar la tabla maestra
            syncWithBackend();
        }
    } catch (e) {
        console.error('Error guardando perfil del estudiante:', e);
